...
export default function EditorDebugger() {
  ...
  const variableDocUrl = `https://docs.openwebgal.com${
    language === 'zhCn' ? '' : `/${language === 'ja' ? 'ja' : 'en'}`
  }/webgal-script/variable.html#%E9%95%BF%E6%95%88%E5%8F%98%E9%87%8F-%E5%85%A8%E5%B1%80%E5%8F%98%E9%87%8F`;
  ...
  const stateEditor = (
    <div>
      <JsonView
        style={{ ...githubLightTheme, overflow: 'auto', fontFamily: '"JetBrains Mono", monospace' }}
  ...
  const addDebugVariable = () =>
    setDebugVariables([...debugVariables, { id: createId(), key: '', value: '', isGlobal: false }]);
    ...
    <div className={s.variablesPanel}>
      <div className={s.variableSection}>
        ...
        <div className={s.variableTable}>
          ...
          {debugVariables.map((item) => (
            <div className={s.variableRow} key={item.id}>
              <Input
                className={s.variableInput}
                size="small"
                value={item.key}
                placeholder={t`变量名`}
                onChange={(_, data) => updateDebugVariable(item.id, { key: data.value })}
              />
              <Input
                className={s.variableInput}
                size="small"
                value={item.value}
                placeholder={t`值或表达式`}
                onChange={(_, data) => updateDebugVariable(item.id, { value: data.value })}
              />
              <Switch
                className={s.variableSwitch}
                label={t`长效`}
                checked={item.isGlobal}
                onChange={(_, data) => updateDebugVariable(item.id, { isGlobal: data.checked })}
              />
              <Button size="small" onClick={() => deleteDebugVariable(item.id)}>{t`删除`}</Button>
            </div>
          ))}
          {debugVariables.length === 0 && (
            <div className={s.variableEmpty}>{t`暂无调试变量，点击“新增变量”创建。`}</div>
          )}
        </div>
      </div>
      <div className={s.variableSection}>
        ...
        <div className={s.variableSnapshot}>
          {gameVars.length > 0 ? (
            gameVars.map(([key, value]) => (
              <div className={s.snapshotRow} key={key}>
                <span>{key}</span>
  ...
  return (
    <div className={s.main}>
      ...
      <div className={s.debuggerContent}>{showComp}</div>
    </div>
  );
}
...
function DebuggerCheckButton(props: { isChecked: boolean; children: ReactNode; onClick: () => void }) {
  return (
    <div className={s.debuggerButton + ' ' + (props.isChecked ? s.checked : '')} onClick={() => props.onClick()}>
      {props.children}
    </div>
  );
}
