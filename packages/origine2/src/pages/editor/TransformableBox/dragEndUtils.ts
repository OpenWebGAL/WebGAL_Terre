 ...
 * @returns
 */
export function getFigureTransformFromFrameInfo(
  direction: string,
  ...
  const temPosition = convertControlToPreview(
    {
      x: frame.translate[0] - ToXOffset(direction, parent, frame.width),
      y: frame.translate[1],
    },
    parent,
  );
  ...
  // 3. 转换旋转和缩放（保持不变）
  const rotationInRadians = Number(degreesToRadians(frame.rotate).toFixed(3));
  ...
  if (Object.keys(newTransformObj).length === 0) {
    return {};
  }
  return newTransformObj;
}
